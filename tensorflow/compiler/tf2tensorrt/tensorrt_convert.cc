...
int do_convert(int argc, char** argv) {
  ...
  std::string inf="/home/tade/data/conv1/pedicle100_save.pb";
  std::string input1="inputs_L";
  std::string output1="inception_v3_3d_1/output/Conv/Relu";

  tensorflow::Status status;
  tensorflow::GraphDef graph_def;
	status = ReadBinaryProto(tensorflow::Env::Default(),inf, &graph_def);
	if (!status.ok()) {
  ...
  graph_properties.InferStatically(true);

 GraphDef* output_graph_def = nullptr;

// Construct ConversionParams.
  const std::vector<string> output_names{output1};
  using tensorflow::tensorrt::convert::ConversionParams;
  ...
  using namespace tensorflow::tensorrt;
  using namespace tensorflow::tensorrt::convert;
  status = ConvertAfterShapes(params);
	if (!status.ok()) {
    	std::cout << status.ToString() << "\n";
     	return 1;
  }
  std::cout << "ConvertAfterShapes-ed: " << status.ToString() << "\n";

  Logger trt_logger;
  tensorflow::tensorrt::convert::ConversionParams cp;
  //tensorflow::tensorrt::convert::Converter conv;
  using tensorflow::tensorrt::convert::Converter;
  using tensorflow::tensorrt::TrtPrecisionMode;
  auto precision_mode = TrtPrecisionMode::FP32;
...
#if GOOGLE_CUDA && GOOGLE_TENSORRT
  return tensorflow::do_convert(argc, argv);
